...
{
  if (not status_led_ptr && not clock_ptr && not console_ptr) {
  ...
  hal::print(*console_ptr, "☠️ TERMINATED ‼️\n");
...
hal::v5::strong_ptr<hal::serial> uart2()
{
  return hal::v5::make_strong_ptr<hal::stm32f1::uart>(
    driver_allocator(), hal::port<2>, hal::buffer<64>);
}
